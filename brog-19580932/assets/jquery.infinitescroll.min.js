...
(function(a, b, c) {
	...
	b.infinitescroll.prototype = {
		...
		_create: function h(a, d) {
			...
			e.loading.msg = b('<div id="infscr-loading"><div>' + e.loading.msgText + "</div></div>");
			...
			e.loading.start = e.loading.start || function() {
				...
				e.loading.msg.appendTo(e.loading.selector).slideDown(e.loading.speed, function() {
					beginAjax(e)
				})
			};
			e.loading.finished = e.loading.finished || function() {
				e.loading.msg.slideUp(e.loading.speed)
			};
...
// disabled as it breaks for collecions like "boots-2" /Fred
...
//				a = a.match(/^(.*?)\b2\b(.*?$)/).slice(1)
		...
		_loadcallback: function l(d, e) {
			var f = this.options, g = this.options.callback, h = f.state.isDone ? "done": !f.appendCallback ? "no-append": "append", i;
		...
		_showdonemsg: function p() {
			...
			a.loading.msg.addClass("done").find("div").html(a.loading.finishedMsg).animate({
				opacity: 1
			}, 2e3, function() {
		...
		retrieve: function v(a) {
			var d = this, e = d.options, f = e.path, g, h, i, j, k, a = a || null, l=!!a ? a : e.state.currPage;
